...
function JellyfinCard({ item, onPlay }) {
  return (
    <div className="shrink-0 w-[140px] sm:w-[160px] cursor-pointer group" onClick={() => onPlay(item)}>
      <div className="relative rounded-xl overflow-hidden bg-secondary aspect-[2/3] mb-2">
        ...
        <WatchlistButton mediaId={`jellyfin:${item.id}`} title={item.title} />
      </div>
...
function JellyfinRow({ title, items, onPlay }) {
  ...
  return (
    <div className="mb-2">
      ...
      <div className="flex gap-3 overflow-x-auto px-4 sm:px-6 pb-2" style={{ scrollbarWidth: 'none' }}>
        {items.map(item => (
          <JellyfinCard key={item.id} item={item} onPlay={onPlay} />
        ))}
      </div>
    </div>
  );
...
export default function JellyfinLibraryViews() {
  ...
  return (
    <>
      {views.map(view => (
        <JellyfinRow key={view.id} title={view.name} items={view.items} onPlay={handlePlay} />
      ))}
    </>
  );
}